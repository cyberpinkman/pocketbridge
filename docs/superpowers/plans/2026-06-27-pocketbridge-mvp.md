...
- Flutter capture now has separate image and file picker actions, covered by `flutter test`, matching the PRD's distinct image/file upload controls.
- Flutter shared-file downloads now stream response chunks directly into a unique local file instead of buffering the entire Mac-to-phone file in memory.
- Flutter shared-file downloads now show per-item downloading state and progress, covered by `flutter test`.
